...
const OrderIndex = () => {
    ...
    const [statusFilter, setStatusFilter] = useState<"Все" | "Новый" | "Отменен">("Все");
    ...
    const cancelOrder = async (id: number) => {
        await axios.put(`http://127.0.0.1:8000/api/orders/${id}/cancel`, {id: id});
        const updatedOrders = orders.map(order => (order.id === id ? {...order, status: 'Отменен' as Status} : order));
    ...
    return (
        <div className='mt-12'>
            ...
            <div className='flex justify-end m-2 p-2'>
                <button onClick={() => setStatusFilter("Все")}
                        className={`px-4 py-2 mr-2 rounded-md ${statusFilter === "Все" ? "bg-indigo-500 text-white" : "text-gray-500"}`}>
                    Все
                </button>
                <button onClick={() => setStatusFilter("Новый")}
                        className={`px-4 py-2 mr-2 rounded-md ${statusFilter === "Новый" ? "bg-indigo-500 text-white" : "text-gray-500"}`}>
                    Новый
                </button>
                <button onClick={() => setStatusFilter("Отменен")}
                        className={`px-4 py-2 mr-2 rounded-md ${statusFilter === "Отменен" ? "bg-indigo-500 text-white" : "text-gray-500"}`}>
                    Отменен
                </button>
            </div>
            <div className="flex justify-end m-2 p-2">
            ...
            <div className="relative overflow-x-auto">
                <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                    {orders.filter(order => {
                        if (statusFilter === "Все" && !startDate && !endDate) return true;
                        if (statusFilter !== "Все" && order.status !== statusFilter) return false;
                        ...
                        <tr key={order.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700">
                            ...
                            <td className="px-6 py-4">
                                <div className="flex items-center">
                                    <div
                                        ...
                                        ${order.status === 'Новый' ? "bg-green-500" :
                                            order.status === "Отменен" ? "bg-red-500" : ""}`}></div>
                                    {order.status}
                                </div>
                            </td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4">
                                {order.status === 'Новый' && (
                                    <button onClick={() => cancelOrder(order.id)} className='text-white bg-red-700
